...
export default function Hero() {
  /* Index of the current slide (0–2) */
  const [current, setCurrent] = useState(0);

  /* Two featured products shown as promo cards on the right */
  ...
  const s = SLIDES[current]; /* Shorthand for current slide */

  return (
    <section className="bg-white px-4 py-4">
      <div className="max-w-7xl mx-auto">
        {/* Two-column: main banner (2/3) + promo cards (1/3) */}
        <div className="grid grid-cols-3 gap-4 items-stretch">
          ...
          >
            <div className="absolute inset-0 p-10 flex flex-col justify-center">
              ...
              {/* Headline — \n renders as line break via whitespace-pre-line */}
              ...
              {/* CTA Button */}
              <button className="fade-up delay-300 bg-primary hover:bg-green-600 text-white font-black text-sm px-6 py-3 rounded-xl w-fit transition-colors border-0 cursor-pointer">
                {s.cta} →
              </button>
            </div>
          ...
          <div className="flex flex-col gap-3">
            {featured.map((p, i) => (
              <div
                key={p.id}
                className={`flex-1 rounded-2xl p-4 flex flex-col justify-between cursor-pointer transition-all hover:-translate-y-1 hover:shadow-green
              ...
              >
                ...
                <div className="flex items-center justify-between mt-2.5">
                  <button className="bg-primary hover:bg-green-600 text-white text-xs font-black px-3 py-1.5 rounded-lg border-0 cursor-pointer transition-colors">
                    Bli Tani
                  </button>
