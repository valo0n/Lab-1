/* ProductDetail — faqe detajesh me reviews + form per review */
  ...
  <span className={size}>
    {[1, 2, 3, 4, 5].map((i) => (
      <span
        ...
        className={`${i <= value ? "text-yellow-400" : "text-gray-200"} ${interactive ? "cursor-pointer hover:scale-110 inline-block transition-transform" : ""}`}
      >
        ★
      </span>
    ))}
...
export default function ProductDetail() {
  ...
  const handleSubmitReview = async () => {
    ...
    try {
      ...
      alert("✅ Review u dërgua! Do të publikohet pas aprovimit nga admin.");
      ...
      <main className="flex-1 max-w-6xl mx-auto w-full px-4 py-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <div className="bg-white rounded-2xl p-10 shadow-card flex items-center justify-center min-h-[400px] relative">
            <span className="text-[200px] select-none">{emoji}</span>
            {disc && (
            ...
            <button
              ...
              className="absolute top-5 right-5 w-12 h-12 bg-white rounded-full flex items-center justify-center shadow hover:bg-red-50 transition-all border-0 cursor-pointer text-2xl"
            >
              {inWishlist ? "❤️" : "🤍"}
            </button>
          </div>
            ...
            {product.sasia_stokut > 0 && (
              <>
                ...
                <button
                  ...
                  className={`w-full text-white font-black py-3 rounded-xl border-0 cursor-pointer transition-all ${
                    added
        ...
        <div className="bg-white rounded-2xl p-6 shadow-card">
          ...
          {reviewsData.reviews.length === 0 ? (
            <div className="text-center py-8 text-muted">
              ...
              <p>Asnjë review ende. Bëhu i pari!</p>
            </div>
          ) : (
